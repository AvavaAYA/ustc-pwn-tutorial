...
关于PWN这一名称的来源，一种广为接受的说法认为其是计算机爆炸时的拟声词。具体来说，我们做pwn题，一般都是为了用符合计算机执行的方式，劫持程序的控制流，最终使得目标的机器运行我们希望运行的代码。

主流的pwn题一般符合这样的模型：

一个含有漏洞的二进制程序，运行在一台出题者提供的远程服务器上，解题者通过远程连接，和服务器上的这个程序收发数据（这种数据一般常常标准输入输出来进行交互，例如常用的printf等等）。解题者通过发送一些构造好的数据，触发服务器上的漏洞，在服务器上执行恶意代码，获得flag信息。

同时，出题者一般会提供这个二进制程序（有些题目中会提供代码），供解题者在本地进行静态漏洞分析和动态调试。

因此，pwn题的大体思想是，在本地分析程序漏洞，并去分析和攻击远程的动态运行环境。所以最基础的一步是在本地进行静态分析。
...
<img src="asm.assets/image-20220301154017356.png" alt="image-20220301154017356" style="zoom:50%;" />
...
<img src="asm.assets/image-20220301154625026.png" alt="image-20220301154625026" style="zoom:50%;" />
